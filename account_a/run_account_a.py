...
Runs the full trading pipeline for the BASELINE paper account.
self_improving: false — no learning injected.
...
# Override data directory to account_a/data/
...
def run():
    ...
    try:
        ...
        theses_data = load_json(os.path.join(PHASE2_DIR, 'data/trade_theses_a.json'),
                     load_json(os.path.join(PHASE2_DIR, 'data/trade_theses.json'), {}))
    ...
    return {
        ...
        'theses_count':  len(load_json(os.path.join(PHASE2_DIR, 'data/trade_theses.json'), {}).get('theses', [])),
    }
