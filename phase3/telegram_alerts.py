...
def send_daily_account_report(stats: dict) -> None:
    """
    ...
    `stats` is the dict returned by reporting.compute_daily_stats().
    """
    label    = stats.get("account", "?")
    ccy      = stats.get("currency", "USD")
    is_b     = "LEARNING" in label or "B" in label.split()[0]
    emoji    = "🟣" if is_b else "⚪"
...
def send_weekly_account_report(stats: dict) -> None:
    """
    Per-account weekly summary covering the most recently completed Mon–Sun
    SGT week (fire Sat morning SGT).

    `stats` is the dict returned by reporting.compute_weekly_stats().
    """
    label  = stats.get("account", "?")
    ccy    = stats.get("currency", "USD")
    is_b   = "LEARNING" in label or "B" in label.split()[0]
    ...
    if best and best is not worst:
        ...
        lines.append(f"  ▲ Best:  {best.get('ticker')} {bp:+.1f}% ({_fmt_money(bu, ccy)}) — {(best.get('exit_reason') or '')[:60]}")
    if worst and worst is not best:
        wp = worst.get("pnl_pct", 0)
        wu = worst.get("_pnl_usd", 0)
        lines.append(f"  ▼ Worst: {worst.get('ticker')} {wp:+.1f}% ({_fmt_money(wu, ccy)}) — {(worst.get('exit_reason') or '')[:60]}")
